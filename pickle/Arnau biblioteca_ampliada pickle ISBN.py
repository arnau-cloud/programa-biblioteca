...
if os.path.exists("Arxius/pickle.txt"):
    with open("Arxius/pickle.txt", "rb") as f:
            llibres = pickle.load(f)
else:
    #Carreguem llibreria estandart
    llibres = [  # Biblioteca amb llibres
        {"Títol": "", "Autor": "", "Any": int(), "Prestat": False, "ISBN": int(), "Data retorn": ""},
        ]
...
while True:  # Bucle infinit fins que es surti del programa
    try:
        ...
        match opcio:  # Mirem quina opció s'ha seleccionat
            ...
            case _:
                raise ValueError
        with open("Arxius/pickle.txt", "wb") as f:
            pickle.dump(llibres, f)
    except ValueError:
        os.system("cls")
        print('-' * term_size.columns)
